...
// ── Auto Code Generator ───────────────────────────────────
function gmGenerateCode(mainId) {
  const pfx = codePrefix[mainId] || 'AS';
  const existing = gmList.filter(g => (g.grpCode || '').startsWith(`${pfx}-`));
  let max = 0;
  existing.forEach(g => {
    const num = parseInt((g.grpCode || '').replace(`${pfx}-`, ''));
    if (!isNaN(num) && num > max) max = num;
  });
  const next = max + 1;
  return `${pfx}-${next < 10 ? '0' + next : next}`;
}
...
async function gmSave() {
  ...
  try {
    ...
    if (res.ok && json.success) {
      gmAlert(id ? 'Group updated successfully!' : 'Group created successfully!');
      gmShowList();
      gmLoadList();
    } else {
...
window.init_group_master = function() { gmLoadList(); };
